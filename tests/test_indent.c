...
int main()
{
 ...
 process = 14;
   ...
   test_buf[0].pid = 0;
   test_buf[0].parent_pid = 0;
   test_buf[0].first_child_pid = 1;
   test_buf[0].next_sibling_pid = process-1;

 for(i=1;i<process-1;i++)
 {
   test_buf[i].pid = i;
   test_buf[i].parent_pid = i-1;
   test_buf[i].first_child_pid = i+1;
   test_buf[i].next_sibling_pid = 0;
 }

   test_buf[process-1].pid = process-1;
   test_buf[process-1].parent_pid = process-2;
   test_buf[process-1].first_child_pid = 0;
   test_buf[process-1].next_sibling_pid = 0;

   test_buf[process].pid = process;
   test_buf[process].parent_pid = 0;
   test_buf[process].first_child_pid = 0;
   test_buf[process].next_sibling_pid = 0;
...
//    printf("Name : %s, Pid: %d,Parent: %d,F.Child: %d,Next: %d\n",test_buf[i].comm,test_buf[i].pid,test_buf[i].parent_pid,test_buf[i].first_child_pid,test_buf[i].next_sibling_pid);
    printf("%d,%d,%d,%d\n",test_buf[i].pid,test_buf[i].parent_pid,test_buf[i].first_child_pid,test_buf[i].next_sibling_pid);
